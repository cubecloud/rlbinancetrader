"""Среда спота Binance long/flat — основа RL-контура (spotrl).

НАЗНАЧЕНИЕ. Gym-среда, поверх которой обучается торговый агент: сначала он
КОПИРУЕТ базовую стратегию v7 бар-в-бар, потом (по мере снятия «лесов»)
становится самостоятельным. Рынок — спот BTC, только long/flat: без плеча,
шортов и фьючерсов; недопустимых действий нет по построению, поэтому масок
действий здесь НЕТ.

ПРОСТРАНСТВО ДЕЙСТВИЙ. MultiDiscrete([2, n_SL, n_TP]):
  голова 0 = {STAY = оставить состояние, FLIP = переключить}. Вне позиции
    FLIP = вход, в позиции FLIP = выход;
  головы SL/TP содержательны ТОЛЬКО на баре входа (на прочих барах их
    значение среда игнорирует), масок для этого не требуется.
Какие свободы агенту разрешены — задаёт FreedomConfig (exit_own/entry_own/
params_own); запертые свободы исполняет механика v7.

ИСПОЛНЕНИЕ и ЭПИЗОД. Решение на баре t исполняется по open(t+1) (П7). Обрыв
эпизода по длине — это truncated=True, НЕ terminated (у эпизода нет
поглощающего состояния); это важно для бутстрапа value в PPO (PLAN 5.2, Т5).

НАГРАДА. Поминутный лог-прирост НАГРАДНОЙ эквити: reward = log(eq_t/eq_{t-1}).
При gamma=1.0 сумма поминутных наград за сделку тождественна log(1+доходность)
(тест Т7); поминутная форма нужна, чтобы решение о ВХОДЕ получало сигнал внутри
короткого rollout-сегмента.

ДВЕ ЭКВИТИ (важно). В среде живут ДВЕ логически разные кривые капитала:
  • НАГРАДНАЯ (`_equity`): старт с 1.0, компаунд, mark-to-market каждую минуту,
    комиссия ДВУСТОРОННЯЯ (вход и выход). По ней считаются награда, слот
    просадки в наблюдении и (в самостоятельном режиме) circuit breaker.
  • МИРОВАЯ v7 (`_world_cash`, ЛЕСА КОПИРОВАНИЯ): считается из книги среды по
    конвенции движка v7 — cash 10M, sizing 0.9999 целыми лотами, ОДНОсторонняя
    комиссия. Нужна ТОЛЬКО чтобы circuit breaker срабатывал бар-в-бар как v7
    (паритет 6/6). Флаг `WorldConfig.cb_equity_mode`: "v7_ledger" (копирование,
    дефолт) — CB по мировой эквити; "reward" (самостоятельный) — CB по наградной
    эквити, мировой леджер НЕ ведётся. Реестр лесов: handoff/scaffolding_registry.

СКРЫТОЕ СОСТОЯНИЕ МИРА (машина в reset/step, П4 — среда считает из СВОЕЙ книги,
не из готовых колонок). Правила v7 с памятью, без которых копия недостижима:
  • post-exit cooldown (N баров после НЕсигнального закрытия, `cooldown_bars`);
  • circuit breaker: взвод по просадке, halt, снятие по смене КАЛЕНДАРНОГО дня,
    переякоривание пика;
  • entered_on_up (вход на растущей ноге) и pos_tag (dip/transition) — на баре
    входа; peak_price и откат от него.
Мутация состояния — ТОЛЬКО в step/book; геттеры `agent_state`/`world_state`
чисты (гейт чистоты Т2). Паритет «среда кладёт в наблюдение те же числа, что
видел эксперт v7» закреплён slow-тестом на обеих эпохах (6/6 разделителей).

Чего в среде НЕТ: второй реализации сигнального тракта v7 (П3). Сравнение с v7
идёт через bridge/, а не через копию стратегии рядом со средой.
