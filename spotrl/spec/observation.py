...
# ВЫБОР РЕАЛИЗАЦИИ — ОСОЗНАННО ЗАФИКСИРОВАН (финализация v2, 2026-07-25). Эти два
# числа доком дословно не заданы, но выбор сделан сознательно и окончательно; они
# ВХОДЯТ В ХЭШ (философия «хэшируем спецификацию»: хэш обязан отражать реально
# посчитанные окна, иначе смена хардкода не сдвинет хэш — молчаливая ложь).
...
#   Побарный режим устраняет свободный параметр окна и любую утечку — это и есть
#   причина окончательного выбора. При отсутствии quote/base (vol=0) → нейтраль
#   (типичная цена (H+L+C)/3).
# - avg_size_median_window_bars=1440 (24h): окно скользящей медианы среднего
#   размера сделки; выбрано равным окну относительного объёма (rolling_median_24h)
#   — одна суточная база нормировки для обоих объёмных признаков, без второго
#   свободного параметра. Зафиксировано окончательно.
SPEC_CONSTANTS_V2_FINALIZED: Dict[str, object] = {
    "vwap_mode": "per_bar_quote_over_base",
    "avg_size_median_window_bars": 24 * 60,
}

# Полный набор констант спецификации (все зафиксированы; draft снят).
SPEC_CONSTANTS_V2: Dict[str, object] = {
    **SPEC_CONSTANTS_V2_VERIFIED, **SPEC_CONSTANTS_V2_FINALIZED}
...
class ObservationSpec:
    ...
    def v2(cls) -> "ObservationSpec":
        ...
        Версия "v2" (финальная, 2026-07-25): состав, порядок колонок И все
        константы нормировки зафиксированы окончательно. Два бывших provisional-
        выбора (vwap_mode="per_bar_quote_over_base" и avg_size_median_window_bars=
        1440) сознательно закреплены как реализация: побарный vwap убирает свободный
        параметр окна и утечку, окно avg_size приравнено к суточному окну объёма.
        Оба входят в хэш — их смена не пройдёт незамеченной.
        """
        return cls(version="v2", market=MARKET_FEATURES_V2,
