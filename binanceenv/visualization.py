...
def visualize_episode(df, asset: Asset):
    ...
    fig.add_trace(go.Scatter(
        ...
        hovertemplate='<b>%{x|%Y-%m-%d %H:%M:%S}</b><br>Reward: %{y:.6f}<extra></extra>'
...
def visualize_episode_1(df, asset: Asset):
    # Create copy to avoid SettingWithCopyWarning
    df = df.copy()
    df['action'] = df['action'].astype(int)

    # Get trade metrics from environment
    trade_metrics = get_trade_metrics(asset.orders.trades)
    active_start, active_end = trade_metrics['active_period']
    ...
    timeframe_label, timeframe_minutes = detect_timeframe(df.index)

    # Calculate Sharpe ratio only for active period
    if active_start and active_end:
        active_df = df.loc[active_start:active_end]
        sharpe_ratio = calculate_sharpe_ratio(active_df['total'], timeframe_minutes)
    else:
        sharpe_ratio = 0.0
    ...
    metrics_text = (
        f"Total Profit: {trade_metrics['total_profit']:.2f}<br>"
        f"Win Rate: {trade_metrics['win_rate']:.2f}%<br>"
        f"Sharpe Ratio ({timeframe_label}): {sharpe_ratio:.2f}<br>"
        f"Max Drawdown: {max_drawdown:.2f}%<br>"
        f"Trades: {trade_metrics['num_trades']}<br>"
        f"Avg Duration: {trade_metrics['avg_trade_duration']}<br>"
