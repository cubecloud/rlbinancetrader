...
class BinanceEnvBase(gymnasium.Env):
    ...
    def __init__(self, data_processor_kwargs: dict, target_balance: float = 100_000., coin_balance: float = 0.,
        ...
        logger.info(f"{self.__class__.__name__} MAX_TIMESTEPS = {self.max_timesteps}")
    ...
    def step(self, action):
        ...
        # TODO: add minimum trading size to check
        msg = str()
        if action == 0:  # sell
    ...
    def reset(self, seed=None, options=None):
        ...
        if self.verbose:
            ...
            msg = (f"Episode length: {self.timecount}\treward {self.reward:.2f}\tAssets: {self.current_balance}\tPNL "
        ...
        if self.ohlcv_df.shape[0] != self.indicators_df.shape[0]:
            msg = (f"{self.__class__.__name__}: ohlcv_df.shape = {self.ohlcv_df.shape}, "
