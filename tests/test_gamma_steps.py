...
def calculate_steps(gamma_values, num_steps):
    ...
    for gamma in gamma_values:
        regress_reward = 0
        steps = 0
        rewards = np.round(np.linspace(-0.5, 0.5, num_steps), 7)  # Round rewards to 5 decimal places
        for reward in rewards:
            regress_reward = reward
            steps = 0
            while abs(regress_reward) > 1e-07:
                regress_reward = gamma * regress_reward
                steps += 1
            steps_to_disappearance.append((reward, gamma, steps))
    return steps_to_disappearance

gamma_values = [0.86, 0.9, 0.92, 0.93, 0.94, 0.95, 0.99, 0.999]  # Test gamma values from 0 to 1
num_steps = 10
