...
class DbEyesSimTxDxMeta(DbEyes):
    """
    Set indicators to working with PredictionHistoryDatabase
    """
    name = 'DbEyesSimTxDxMeta'
    ind_params = dbeyessimv1_algoparams_1h_t40
    timeframe = '5m'
    discretization = '15m'
    ...
    def decision(self) -> list:
        ...
        if minus_signals:
            ...
            if total_minus_signal_prediction >= self.total_indicator['minus']['threshold']:
                todo_lst.append(('close',
                                 ceil_time(target_time, ceil_to=self.timeframe),
                                 self.total_indicator['minus'],
                                 None,
                                 ))
        ...
        if plus_signals:
            ...
            if total_plus_signal_prediction >= self.total_indicator['plus']['threshold']:
                todo_lst.append(('buy',
                                 ceil_time(target_time, ceil_to=self.timeframe),
                                 self.total_indicator['plus'],
                                 None,
    ...
    def get_current_datetime_todo_orders(self, side: str) -> list:
        todo_orders: list = []
        for todo_order in self.orders_stack:
            if todo_order[1] == self.current_datetime and side == todo_order[0]:
                todo_orders.append(todo_order)
        return todo_orders
    ...
    def process_orders(self):
        ...
        if self.position.size == 0.0:
            power_trend = -np.inf
            buy_todo_orders = self.get_current_datetime_todo_orders('buy')
            chosen_todo_order = None
            if buy_todo_orders:
                for todo_order in buy_todo_orders:
                    if todo_order[3] is not None:
                        if todo_order[3] > power_trend:
                            chosen_todo_order = copy.deepcopy(todo_order)
                            power_trend = float(todo_order[3])
                    else:
                        chosen_todo_order = copy.deepcopy(todo_order)
                        break
                buy_kwargs: dict = {}
                self.position.entry_price = current_price
                sl = current_price - (current_price * chosen_todo_order[2]['stop_loss'])
                tp = current_price + (current_price * chosen_todo_order[2]['take_profit'])
                buy_kwargs.update({"sl": sl,
                                   "tp": tp})
                self.buy(**buy_kwargs)
        elif self.position.is_long:
            close_todo_orders = self.get_current_datetime_todo_orders('close')
            if close_todo_orders:
                self.position.close()
        self.orders_stack = [todo_order for todo_order in self.orders_stack if todo_order[1] > self.current_datetime]
...
class DbEyesSimT5D15(DbEyesSimV1):
    """
    Set indicators to working with PredictionHistoryDatabase
    """
    name = 'DbEyesSimT5D15'
...
class DbEyesSLOnlyT15D30(DbEyesSimV1):
    """
    Set indicators to working with PredictionHistoryDatabase
    """
    name = 'DbEyesSLOnlyT15D30'
    ind_params = dbeyessimv1t30_algoparams_59573
    timeframe = '15m'
    discretization = '30m'
