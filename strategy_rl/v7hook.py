...
class AgentHookBT(RegimeDipBuyerBT):
    ...
    # ВНИМАНИЕ (критика 0.3, находка №1): сырой exit_policy видит весь
    # объект стратегии, включая _next_open (будущее!). Он разрешён ТОЛЬКО
    # oracle-политикам этапа 0.5. Обучаемые политики подключаются строго
    # через strategy_rl.policyio.IsolatedExitPolicy (белый список фич).
    exit_policy = None
    # ЗАГОТОВКИ будущих свобод (беклог «постепенная свобода», 23.07):
    # пока None и НЕ подключены к next() — включение каждой = отдельное
    # pre-reg решение с собственным дизайном вызова.
    entry_policy = None       # право пропустить/инициировать вход
    transition_policy = None  # действия на transition-сделках
