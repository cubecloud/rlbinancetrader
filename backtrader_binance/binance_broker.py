...
class BinanceOrder(OrderBase):
    (Market, Close, Limit, Stop, StopLimit, StopTrail, StopTrailLimit,
     Historical, TakeProfit, TakeProfitLimit) = range(10)
    ExecTypes = ['Market', 'Close', 'Limit', 'Stop', 'StopLimit', 'StopTrail',
                 'StopTrailLimit', 'Historical', 'TakeProfit', 'TakeProfitLimit']
...
class BinanceBroker(BrokerBase):
    _ORDER_TYPES = {
        ...
        BinanceOrder.TakeProfit: ORDER_TYPE_TAKE_PROFIT,
        BinanceOrder.TakeProfitLimit: ORDER_TYPE_TAKE_PROFIT_LIMIT,
    }
    ...
    def get_all_orders(self, symbol):
        return self._store.get_all_orders(symbol=symbol)

    def get_open_orders(self, symbol):
        return self._store.get_open_orders(symbol=symbol)

    def get_trade_fee(self, symbol):
        return self._store.get_trade_fee(symbol)

    def get_avg_price(self, symbol):
        return float(self._store.get_avg_price(symbol)['price'])
